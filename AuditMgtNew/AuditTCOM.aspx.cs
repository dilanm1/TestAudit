...
{
    public partial class AuditTCOM : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (this.Page.PreviousPage != null)
            {
                ...
                ExaminationTC exam = new ExaminationTC(Int32.Parse(Session["mid"].ToString()), Int32.Parse(sid), sname, location);
                exam.GetQuestions();
        ...
        public void DisplayQuestion()
        {
            ...
            ExaminationTC e = (ExaminationTC)Session["questions"];
            // display data
        ...
        {
            ExaminationTC exam = (ExaminationTC)Session["questions"];
        ...
        protected void btnNext_ServerClick(object sender, EventArgs e)
        {
            ...
            //      cmd.CommandText = "insert into tblSavedAnswers values(@mid,@examid,@location,@sid,@qid,@question,@answer,@evidence,@comments)";
            ...
            //    SqlConnection con4 = new SqlConnection(DBUtil.ConnectionString);
            //    SqlDataAdapter da = new SqlDataAdapter("select oe_subjects.sid, oe_subjects.sname,oe_subjects.weighting,oe_subjects.status, oe_subjects.score FROM oe_subjects INNER JOIN tbllocation ON oe_subjects.locationid = tbllocation.locationid where tbllocation.location  = '" + exam.location.ToString() + "' ", con4);
            //    DataSet ds = new DataSet();
            //    da.Fill(ds, "questions");
            ...
            //    //SqlCommand cmd2 = new SqlCommand("select isnull( max(examid),0) + 1 from oe_exams", con2);
            //    //int examid = (Int32)cmd2.ExecuteScalar();
            //    SqlCommand cmd3 = new SqlCommand("UPDATE oe_subjects SET oe_subjects.score = @score,oe_subjects.status =@status FROM oe_subjects INNER JOIN tbllocation ON oe_subjects.locationid = tbllocation.locationid where tbllocation.location =@loc and oe_subjects.sid=@sid ", con6);
            //    // cmd2.CommandText = ("Update oe_subjects Set status=@status WHERE sid =@sid",con2);
            //    cmd3.Parameters.Add("@score", SqlDbType.Float).Value = exam.score;
            //    cmd3.Parameters.Add("@status", SqlDbType.NVarChar).Value = "complete";
            ...
            if (exam.curpos == exam.SIZE - 1)
            {
                ...
                Response.Redirect("TCIndicators.aspx");
        ...
        protected void ImageButton8_ServerClick(object sender, EventArgs e)
        {
            ...
            //SqlCommand cmd2 = new SqlCommand("select isnull( max(examid),0) + 1 from oe_exams", con2);
            //int examid = (Int32)cmd2.ExecuteScalar();
            SqlCommand cmd2 = new SqlCommand("UPDATE oe_subjects SET oe_subjects.status =@status FROM oe_subjects INNER JOIN tbllocation ON oe_subjects.locationid = tbllocation.locationid where tbllocation.location=@loc and oe_subjects.sid=@sid", con2);
            // cmd2.CommandText = ("Update oe_subjects Set status=@status WHERE sid =@sid",con2);
