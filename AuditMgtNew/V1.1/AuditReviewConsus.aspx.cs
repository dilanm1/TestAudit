...
{
    public partial class AuditReviewConsus : System.Web.UI.Page
    {
        ...
        protected void btnNext_ServerClick(object sender, EventArgs e)
        {
            ...
            // Response.Redirect("showresult.aspx");
            {
                foreach (Question q in exam.questions)
                {
                    ...
                    cmd.CommandText = "insert into tblSavedAnswersConsus values(@examid,@location,@mid,@sid,@qid,@question,@answer,@evidence,@comments)";
                ...
                //SqlDataAdapter da = new SqlDataAdapter("select * from oe_subjects_corporate where sid = " + q3.sid, con4);
        ...
        protected void ImageButton8_ServerClick(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Session.Contents.Count; i++)
            {
                ...
                SqlCommand cmd = new SqlCommand("select isnull( max(examid),0) + 1 from oe_exams", con);
                int examid = (Int32)cmd.ExecuteScalar();
                cmd.CommandText = "insert into tblSavedAnswersConsus values(@mid,@location,@examid,@sid,@qid,@question,@answer,@evidence,@comments)";
            ...
            //SqlCommand cmd2 = new SqlCommand("select isnull( max(examid),0) + 1 from oe_exams", con2);
            //int examid = (Int32)cmd2.ExecuteScalar();
            SqlCommand cmd2 = new SqlCommand("UPDATE View_subject_location SET View_subject_location.status=@status where View_subject_location.sid=@sid and View_subject_location.location=@loc", con2);
            ...
            //SqlCommand cmd2 = new SqlCommand("select isnull( max(examid),0) + 1 from oe_exams", con2);
            //int examid = (Int32)cmd2.ExecuteScalar();
            SqlCommand cmd3 = new SqlCommand("Delete from tblSavedAnswers where location=@loc and sid=@sid", con3);
            // cmd2.CommandText = ("Update oe_subjects Set status=@status WHERE sid =@sid",con2);
